...
fn validate_stream(mut stream: &[u8]) -> Result<&[u8]> {
    ...
    if &stream[..MAGIC.len()] != MAGIC {
        bail!(ErrorKind::SnappyError(snap::Error::Header));
    }
...
impl<'a> SnappyReader<'a> {
    ...
    fn _read(&mut self, buf: &mut [u8]) -> Result<usize> {
        if self.uncompressed_pos < self.uncompressed_chunk.len() || self.next_chunk()? {
...
macro_rules! to_io_error {
    ($expr:expr) => {
        match $expr {
            ...
            Err(Error(ErrorKind::IoError(io_error), _)) => Err(io_error),
